...
public class NAKACK2 extends Protocol implements DiagnosticsHandler.ProbeHandler {
    ...
    public Object up(Event evt) {
        if(evt.type() == Event.STABLE) {  // generated by STABLE layer. Delete stable messages passed in arg
            stable(evt.getArg());
    ...
    protected void overwriteDigest(Digest digest) {
        ...
        for(Digest.Entry entry: digest) {
            Address member=entry.member();
            if(member == null)
                continue;

            long highest_delivered_seqno=entry.hd();
